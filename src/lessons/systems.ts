...
export const systemsLesson: LessonDefinition = {
  ...
  learningObjectives: [
    ...
    "See why 'exactly one solution for every $\\mathbf{b}$' is the same statement as invertibility ($\\det A \\neq 0$)",
  ],
  motivatingQuestion:
  ...
  formalBlocks: [
    ...
    },
    {
      ...
      statement:
        "For a $2\\times 2$ system: if the columns are **independent** ($\\det A \\neq 0$), then $A\\mathbf{x} = \\mathbf{b}$ has **exactly one** solution for *every* $\\mathbf{b}$. If the columns are **dependent** ($\\det A = 0$), then for a given $\\mathbf{b}$ there are **either none or infinitely many** — never exactly one.",
      interpretation:
        "Independence controls *uniqueness*; the target controls *existence*. 'Exactly one solution for every $\\mathbf{b}$' is precisely what it means for $A$ to be **invertible** — and that is the boundary the next lesson's determinant is built to detect.",
      visibility: "revealed",
      layers: [
        {
          kind: "connection",
  ...
  workedExamples: [
    {
      ...
      equations: [
        ...
        "\\det A = (1)(-1) - (3)(2) = -7 \\neq 0 \\;\\Rightarrow\\; \\text{unique for every } \\mathbf{b}",
      ],
      equationsAriaLabel:
        "Solving the independent system by rows to get x equals two and y equals minus one, confirming by the column combination two a-one minus a-two equals b, and noting the determinant is minus seven, nonzero, so the solution is unique.",
      layers: [
        {
          kind: "connection",
    ...
    },
    {
      ...
      equations: [
        ...
        "\\det A = (1)(4) - (2)(2) = 0 \\;\\Rightarrow\\; \\text{columns dependent}",
      ...
      equationsAriaLabel:
        "With dependent columns, the two equations collapse to one, the determinant is zero, the target lies on the span of the columns, and the solution set is the whole line x equals three minus two y — infinitely many solutions.",
    },
    {
      id: "sys-none",
  ...
  callouts: [
    ...
    },
    {
      ...
      resolve:
        "What controls uniqueness is whether the constraints are *independent* — equivalently, whether the columns are independent, $\\det A \\neq 0$ — not how many equations you wrote.",
    },
  ],
  checkpoint: {
    prompt:
  ...
  exercises: [
    {
      ...
      tier: "check",
      prompt:
        "How many solutions does $\\begin{cases} x + 2y = 3 \\\\ 2x + 4y = 6 \\end{cases}$ have?",
      choices: [
        "Exactly one",
    ...
    },
    {
      ...
      tier: "check",
      prompt:
        "How many solutions does $\\begin{cases} x + 2y = 3 \\\\ 2x + 4y = 5 \\end{cases}$ have?",
      choices: [
        "Infinitely many",
    ...
    },
    {
      ...
      tolerance: 0.01,
      explanation:
        "From row 2, $y = 2x - 5$; substituting gives $7x = 14$, so $x = 2$, $y = -1$. Equivalently $2\\,\\mathbf{a}_1 - \\mathbf{a}_2 = (-1, 5)$ in the column picture. The columns are independent ($\\det A = -7$), so this is the only solution.",
    },
    {
      ...
      choices: [
        ...
        "Compute the determinant of $A$ and read its sign",
      ],
      correctChoice: 1,
      explanation:
    ...
    {
      id: "sys-predict-every-b",
      type: "prediction",
      prompt:
        "Predict: if the columns of $A$ are independent, does $A\\mathbf{x} = \\mathbf{b}$ have a solution for *every* target $\\mathbf{b}$? Does it ever have more than one?",
      reveal:
        "Yes to existence, no to multiplicity. Independent columns span the whole plane, so every $\\mathbf{b}$ is reachable (consistent). And independence means the combination reaching $\\mathbf{b}$ is unique — exactly Lesson 1's 'independent pair ⇒ unique coordinates'. So there is exactly one solution for every $\\mathbf{b}$, which is what invertibility means.",
    },
    {
      id: "sys-construct-none",
      type: "multiple-choice",
      tier: "transfer",
      prompt:
        "The columns are $\\mathbf{a}_1 = (1, 2)$ and $\\mathbf{a}_2 = (2, 4)$ (dependent). Which target $\\mathbf{b}$ makes $A\\mathbf{x} = \\mathbf{b}$ have **no** solution?",
      choices: [
        "$\\mathbf{b} = (2, 4)$",
        "$\\mathbf{b} = (3, 6)$",
        "$\\mathbf{b} = (1, 1)$",
      ...
      correctChoice: 2,
      explanation:
        "The columns span only the line through $(1, 2)$. Targets $(2,4)$, $(3,6)$, and $(0,0)$ all lie on that line (reachable — infinitely many solutions each). Only $(1, 1)$ is off the line, so it is unreachable: no solution.",
    },
    {
      ...
      choices: [
        "$\\mathbf{b} = \\mathbf{0}$",
        "the columns of $A$ are independent ($\\det A \\neq 0$)",
        "$A$ is symmetric",
        "the entries of $A$ are large",
      ],
      correctChoice: 1,
      explanation:
        "Independent columns span the plane (existence for every $\\mathbf{b}$) and give unique combinations (uniqueness). That is exactly invertibility, and $\\det A \\neq 0$ is the single number that certifies it — the bridge to the next lesson.",
    },
    {
      id: "sys-det-zero",
      type: "numeric",
      tier: "drill",
      prompt:
        "Compute $\\det\\begin{bmatrix} 1 & 2 \\\\ 2 & 4 \\end{bmatrix}$ (the dependent example). Enter the number.",
      expected: 0,
      tolerance: 1e-9,
      explanation:
        "$\\det = (1)(4) - (2)(2) = 0$. A zero determinant flags dependent columns — the plane collapses onto a line — which is exactly why this system is either inconsistent or has infinitely many solutions, never a unique one.",
    },
  ],
  keyTakeaway:
    "$A\\mathbf{x} = \\mathbf{b}$ is one equation with two pictures: rows are lines whose intersection is the solution, columns are arrows you blend to reach $\\mathbf{b}$. A solution exists exactly when $\\mathbf{b}$ is in the span of the columns; it is unique exactly when the columns are independent ($\\det A \\neq 0$). Independent ⇒ one solution for every $\\mathbf{b}$ (invertible); dependent ⇒ none or infinitely many, depending on $\\mathbf{b}$.",
};
