 ...
 * but never routed through `ProseWithMath` — it is consumed as a plain-text
 * label/tooltip, e.g. an HTML `title` attribute). It intentionally carries no
 * `$...$` KaTeX and no `**bold**`, so it is exempt from the bold-straddling-math
 * failure mode documented in `docs/quality/known-failure-modes.md`.
 */
