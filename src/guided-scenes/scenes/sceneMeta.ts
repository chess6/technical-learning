...
export const SCENE_META: Record<string, GuidedSceneMeta> = {
  ...
  "eigenvectors-characteristic-equation": {
    ...
    size: SCENE_SIZE,
    ariaLabel:
      "Guided bridge showing why the eigenvalues are the roots of the characteristic determinant. Lambda is a dial. Subtracting lambda down the diagonal gives a whole family of maps, A minus lambda I, and the plane deforms under whichever member the dial selects while the graph of the determinant against lambda is traced by the same sweep. As lambda reaches two the curve crosses the axis and the grid flattens onto a line at the same instant: the line the whole plane lands on is the image, and the different line of inputs sent to the origin is the kernel, each named for the space it lives in, with a probe walking the kernel while its image stays on the origin. Between the two roots the determinant is negative and the plane is not flat. At lambda equals three the curve crosses zero again, coming back up, and the kernel and image swap places. Both roots are simple, so the curve passes through the axis at each rather than touching it. Only then is the traced curve named as a quadratic whose roots are the eigenvalues, and back under A both eigendirections share the frame, each stretched by its own eigenvalue.",
  ...
  "optimization-approximation": {
    ...
    size: SCENE_SIZE,
    ariaLabel:
      "Guided animation for deciding with the derivative. F of x equals x cubed minus three x is plotted on the interval from minus two to three; sampled values appear, and the largest sample is flagged as not yet an answer. A point is placed where the slope is nonzero, with its local-linear model drawn. A step is taken in the improving direction, held before reveal; the linear term and the residual error are drawn as two separately labelled quantities, with the linear term dominating. The step is then enlarged until the predicted and actual signs genuinely disagree, with a live agreement readout — the animation states this is a radius that works, never the largest one. A sweep then runs across the whole interval: every point where the slope is nonzero greys out, leaving four candidates. The animation flags this sweep as a sampled visualization of a proven theorem, not a test of every real point. The scene resets to x cubed at zero: flat, held before reveal, then shown to be neither a maximum nor a minimum — a survivor, not an answer. It resets again to the absolute value of x on minus two to two, showing the minimum sitting exactly where no local model exists, a point the sweep's argument cannot examine. It resets once more to the left endpoint of the main interval, where a one-sided line shows only one direction is available: stepping right refutes a local maximum, but stepping left would leave the domain, so a local minimum is never tested there. Finally the scene returns to the main function and compares the full candidate set: the global maximum sits at the right endpoint, not at the interior local maximum.",
