...
function objectIsRegistered(
  ...
  return !!prefix && registered.some((candidate) => candidate.startsWith(prefix));
}
...
export function validateBeatContract(contract: SceneBeatContract): string[] {
  ...
  if (!segments) return [`${contract.sceneId}: no production timing registry`];
  ...
  if (productionIds.join("\0") !== beatIds.join("\0")) {
    problems.push(`${contract.sceneId}: BeatSpec ids must match production segment order`);
  }

  const visitObject = (beatId: string, objectId: SemanticObjectId) => {
    if (!objectIsRegistered(objectId, contract.semanticObjects)) {
      problems.push(`${contract.sceneId}.${beatId}: unregistered object ${objectId}`);
    }
  };

  for (const beat of contract.beats) {
    if (!beat.purpose.trim()) problems.push(`${contract.sceneId}.${beat.id}: purpose is empty`);
    if (beat.chapter.id !== beat.id) {
      problems.push(`${contract.sceneId}.${beat.id}: chapter id must match beat id`);
    }
    ...
    for (const required of ["opening", "midpoint", "landing", "final"]) {
      if (!checkpointIds.has(required)) {
        problems.push(`${contract.sceneId}.${beat.id}: missing ${required} checkpoint`);
      }
    }
    const phases = SCENE_BEATS[contract.sceneId]?.[beat.id] ?? {};
    for (const checkpoint of beat.checkpoints) {
      if (checkpoint.anchor.kind === "phase" && !(checkpoint.anchor.phaseId in phases)) {
        problems.push(`${contract.sceneId}.${beat.id}.${checkpoint.id}: unknown phase ${checkpoint.anchor.phaseId}`);
      }
      for (const objectId of checkpoint.requiredObjects ?? []) visitObject(beat.id, objectId);
    }

    for (const objectId of beat.focalObjects) visitObject(beat.id, objectId);
    for (const objectId of beat.expectedStableObjects ?? []) visitObject(beat.id, objectId);
    for (const change of beat.expectedChanges ?? []) visitObject(beat.id, change.objectId);

    const changes = beat.expectedChanges ?? [];
    if (beat.intent === "hold" && changes.length > 0) {
      problems.push(`${contract.sceneId}.${beat.id}: hold cannot declare changes`);
    }
    if (beat.intent === "geometry") {
      if (!changes.some(({property}) => property === "geometry")) {
        problems.push(`${contract.sceneId}.${beat.id}: geometry needs geometry change`);
      }
    } else if (changes.some(({property}) => property === "geometry")) {
      problems.push(`${contract.sceneId}.${beat.id}: ${beat.intent} cannot satisfy geometry`);
    }
    if (beat.intent === "camera") {
      if (!changes.some(({property}) => property === "camera")) {
        problems.push(`${contract.sceneId}.${beat.id}: camera needs camera change`);
      }
    } else if (changes.some(({property}) => property === "camera")) {
      problems.push(`${contract.sceneId}.${beat.id}: ${beat.intent} cannot satisfy camera`);
    }
    for (const change of changes) {
      if ((change.property === "geometry" || change.property === "camera") && change.continuous !== true) {
        problems.push(`${contract.sceneId}.${beat.id}: ${change.objectId} ${change.property} must be continuous`);
      }
      if (beat.expectedStableObjects?.includes(change.objectId)) {
        problems.push(`${contract.sceneId}.${beat.id}: ${change.objectId} is both changed and stable`);
      }
    }

    if (beat.prediction && !beatIds.includes(beat.prediction.revealBeat)) {
      problems.push(`${contract.sceneId}.${beat.id}: reveal beat ${beat.prediction.revealBeat} is missing`);
    }
  }
  return problems;
}
