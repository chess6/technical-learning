...
The [Insight Discovery Gate](./INSIGHT_DISCOVERY_GATE.md) answers a *different*,
...
> [LESSON_MASTERY_CONTRACT.md](./LESSON_MASTERY_CONTRACT.md); the course-level
> calibration benchmarks live in
> [LINEAR_ALGEBRA_BENCHMARK_MATRIX.md](./LINEAR_ALGEBRA_BENCHMARK_MATRIX.md);
> presentation lives in [SEMANTIC_PAGE_GRAMMAR.md](./SEMANTIC_PAGE_GRAMMAR.md).
...
- [ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md) — the working *library* of
...
- The per-lesson decision of *which* dimensions apply is recorded in the
  [Lesson Mastery Contract](./LESSON_MASTERY_CONTRACT.md); the per-module and
  per-course decisions are recorded in the
  [benchmark matrix](./LINEAR_ALGEBRA_BENCHMARK_MATRIX.md) and module assessment
  plans.
...
`check | drill | transfer` [exercise tiers](./ASSESSMENT_PATTERNS.md#tiers--phases)
and the `recall / reconstruction / transfer` levels of the
[Insight Validation Protocol](./INSIGHT_VALIDATION_PROTOCOL.md) — rather than
...
[ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md), and the *per-lesson* exercise
...
[Insight Validation Protocol](./INSIGHT_VALIDATION_PROTOCOL.md), whose cohort
...
and the [Lesson Mastery Contract](./LESSON_MASTERY_CONTRACT.md) and
[Course Spine](./LINEAR_ALGEBRA_COURSE_SPINE.md) enforce, that:
...
- **Notation consistency** — one symbol set across prose, scenes, explorers,
  exercises (LESSON_DESIGN.md notation rules; MATH_CORRECTNESS.md).
- **Recurring mathematical objects** — reuse the shared canonical examples
  ([CURRICULUM_ARCHITECTURE.md §4](./CURRICULUM_ARCHITECTURE.md)) so an edge is
...
- **Controlled abstraction increase** — abstraction rises deliberately along the
  [abstraction path](./SEMANTIC_PAGE_GRAMMAR.md#4-mathematical-object-and-representation-standard);
...
- **Explicit dependency relationships** — the prerequisite DAG in
  [CURRICULUM_ARCHITECTURE.md §2](./CURRICULUM_ARCHITECTURE.md) is the source of
...
| 2 | **Spine + dependency map** | Order topics as re-interpretations of prior ones; fix prerequisites. | Profile; existing [spine](./LINEAR_ALGEBRA_COURSE_SPINE.md) + [DAG](./CURRICULUM_ARCHITECTURE.md). | Updated spine row + prerequisite edges + concept ids. | A lesson's prerequisites aren't taught earlier; cycle in the DAG. |
| 3 | **Insight Discovery Brief** | Diagnose the obstacle; generate & rank insight candidates. | Spine insight (as *inherited hypothesis*, not answer). | `docs/insight-brief-<topic>.md`. | See [Insight Gate Stage 1](./INSIGHT_DISCOVERY_GATE.md#stage-1--insight-discovery-brief). |
| 4 | **Approved Insight Contract** | Commit to one insight; prove its chain; run audits. | The brief. | `docs/insights/<topic>.md` ending `Gate result: PASS`. | See [Insight Gate Stage 2](./INSIGHT_DISCOVERY_GATE.md#stage-2--approved-insight-contract). |
| 5 | **Lesson Mastery Contract** | Guarantee coverage, rigor, practice, transfer, retention around the insight. | The `PASS` contract + profile + spine row. | A completed [Lesson Mastery Contract](./LESSON_MASTERY_CONTRACT.md). | Any contract [rejection condition](./LESSON_MASTERY_CONTRACT.md#5-rejection-conditions-the-mastery-gate). |
| 6 | **Lesson spec + authoring** | Build the lesson from the block palette + page grammar. | Mastery contract; [LESSON_TEMPLATE.md](./LESSON_TEMPLATE.md); [LESSON_DESIGN.md](./LESSON_DESIGN.md); [SEMANTIC_PAGE_GRAMMAR.md](./SEMANTIC_PAGE_GRAMMAR.md). | Typed `LessonDefinition` + scenes + explorers + exercises. | Missing must-teach content; page leads with a purposeless visual (grammar §3). |
| 7 | **Mathematical correctness review** | Ensure every shown quantity is mathematically correct. | The built lesson. | Completed [LESSON_CORRECTNESS_CHECKLIST.md](./LESSON_CORRECTNESS_CHECKLIST.md) + [MATH_CORRECTNESS.md](./MATH_CORRECTNESS.md) sign-off. | Any unverified/plausible-but-wrong math; missing regression test. |
...
| 10 | **Course benchmark validation** | Check the whole course against the profile benchmark. | All modules; the [benchmark matrix](./LINEAR_ALGEBRA_BENCHMARK_MATRIX.md). | A benchmark validation report (covered / missing / deferred). | Claims a profile the assessments don't support; silent coverage gaps. |
...
| **Presentation (how it reads)** | [LESSON_DESIGN.md](./LESSON_DESIGN.md) + [SEMANTIC_PAGE_GRAMMAR.md](./SEMANTIC_PAGE_GRAMMAR.md) | Block palette, page grammar, representation choice, prose. |
| **Content & sequence (what & when)** | [LINEAR_ALGEBRA_COURSE_SPINE.md](./LINEAR_ALGEBRA_COURSE_SPINE.md) + [CURRICULUM_ARCHITECTURE.md](./CURRICULUM_ARCHITECTURE.md) + [LINEAR_ALGEBRA_BENCHMARK_MATRIX.md](./LINEAR_ALGEBRA_BENCHMARK_MATRIX.md) | Topics, order, prerequisites, external calibration. |
| **Correctness** | [MATH_CORRECTNESS.md](./MATH_CORRECTNESS.md) + [ERROR_LOG.md](./ERROR_LOG.md) + [LESSON_CORRECTNESS_CHECKLIST.md](./LESSON_CORRECTNESS_CHECKLIST.md) | Mathematical + visualization correctness. |
| **Assessment library & pilot** | [ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md) + [INSIGHT_VALIDATION_PROTOCOL.md](./INSIGHT_VALIDATION_PROTOCOL.md) | Reusable patterns; post-ship learner validation. |
| **Data model** | [MULTI_DOMAIN_CURRICULUM_ARCHITECTURE.md](./MULTI_DOMAIN_CURRICULUM_ARCHITECTURE.md) + `learnerState.ts` + `capabilities.ts` | Curriculum tree/graph; progress envelope; grading. |
