...
| **B** | **IN PROGRESS — L5 `chain-rule` and L6 `optimization-approximation` are built, merged to `master`, and Gate-8 accepted (2026-08-01 and 2026-08-10). L7 `substitution-parts` CLAIMED on `feature/l7-substitution-parts`: corrected Gate-3 brief; Gate-4 contract DRAFTED, marked NOT YET PASS (its adversarial audit was cut short by a session limit and must run before Gate 5); math layer (`src/math/integrationTechniques.ts`, 20 tests) built ahead under the standing ADR-008 authorization. L8: two RAW unjudged Gate-3 draft directions preserved in `lessons/08-improper-integrals/insight-brief-drafts.md`; no brief yet.** | `feature/l7-substitution-parts` | L5: [contract](lessons/05-chain-rule/mastery-contract.md#6-acceptance-record-gate-8) · L6: [contract](lessons/06-optimization-approximation/mastery-contract.md#6-acceptance-record-gate-8) · L7: [Gate-3 brief](lessons/07-substitution-parts/insight-brief.md) · [Gate-4 draft](lessons/07-substitution-parts/insight.md) |
