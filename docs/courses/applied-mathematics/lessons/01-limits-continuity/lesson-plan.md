...
| --- | --- | --- | --- | --- |
| 1 | `lim-diagnose-definition` — decide existence and name the failure on four fresh functions, **from their definitions** | `exercise-sequence`, multiple-choice classification (E2; converted from typed text 2026-07-30) | exists / jump / oscillation / blow-up | Points at which band cannot be answered, and why one-sided limits settle the jump case. *(Graph reading is practised in the explorer; no graded item captures it — see the contract's §1e.)* |
| 2 | `lim-point-value-irrelevant` — say what changes if \(f(a)\) is altered | `exercise-sequence`, multiple-choice (E2; converted from typed text 2026-07-30) | "still 7 — unchanged", for both the changed-value and deleted-value steps | Re-shows the puncture beat: the definition consults \(0<\lvert x-a\rvert\). |
...
| 6 | `lim-repair-transfer` — can changing one point repair the limit, continuity, both, neither? | `exercise-sequence` on an unfamiliar function, all steps multiple-choice (E2; converted from typed yes/no 2026-07-30) | continuity only | The limit is fixed by the neighbours and cannot be edited at a point. |
| 7 | `lim-why-substitution-works` — justify substituting into a polynomial | `multiple-choice` (E2; converted from typed text 2026-07-30) | cites continuity | Rejects "because it's easy"; requires the property to be named. |
| 7b | `lim-continuity-not-enough` — the samples are all near zero; may you conclude the function is? | `exercise-sequence` (verdict → what would have to be added), both steps multiple-choice (E2; converted from typed judgments 2026-07-30) | no; a resolution **and** a modulus | Shows `ex-hidden-spike`; names the two missing ingredients. |
