...
[courses/linear-algebra/course-spine.md](course-spine.md) answers
...
- [courses/linear-algebra/course-spine.md](course-spine.md) — the
...
- [courses/multi-domain-architecture.md](../multi-domain-architecture.md)
  ...
  model's §3 defers).
- [authoring/lesson-design.md](../../authoring/lesson-design.md) / [product/vision.md](../../product/vision.md)
...
> the built lessons (§1 Status column). Listing the `future` lessons here — and
> recommending the next unbuilt node in §6 — does not authorize building them
> without an explicit reopen. Concept ids and edges are a *commitment* so the
> pieces land coherently when each lesson is actually built.
...
`courses/multi-domain-architecture.md` §3 defers to the platform model). An
...
(`authoring/lesson-design.md` "Guided-to-interactive continuity"). Reusing the *same
...
purely to catch the math-visualization bugs enumerated in `quality/known-failure-modes.md`.

---

## 5. Which lesson proves each platform feature

The platform capabilities (block palette in `authoring/lesson-design.md`; exercise
