...
| 9 | **Cumulative module assessment** | Certify integration + delayed retention across the module, and **discharge module-owned outcomes** with real evidence. | All accepted lessons in the module + their module-owned obligations. | `docs/courses/<course>/modules/<module>/assessment-plan.md` (from [module-assessment-plan.md](templates/module-assessment-plan.md)) — cumulative/interleaved/spaced set + results verifying the module-owned outcomes and any module-owned abstraction-returns. | No cumulative/delayed items (case #9); no method-selection items; a module-owned outcome claimed mastered without real results. |
| 10 | **Benchmark validation** | Check each module, then the whole course, against the profile benchmark and the declared target. | All modules; the [benchmark matrix](../courses/linear-algebra/benchmark-matrix.md); the [declared target](../courses/linear-algebra/course-spine.md#0-declared-course-target-gate-1). | `docs/courses/<course>/modules/<module>/validation.md` (from [module-validation.md](templates/module-validation.md)) per module, rolled up into a course report (covered / missing / deferred). | Claims a profile the assessments don't support; silent coverage gaps. |
