...
(`INTERACTIVE_TEXTBOOK_VISION.md` §14): **Chapter 0 demonstrates surprising
...
transition, not a matrix factorization; `LESSON_DESIGN.md` animation-choreography,
`animation-quality-bar.md`).
...
- `docs/LESSON_CORRECTNESS_CHECKLIST.md` — completed for the three parts.
...
6. **Polish & docs.** HomePage copy; sidebar course-relative numbering; complete
   `LESSON_CORRECTNESS_CHECKLIST.md`; `npm run lint`, `npm run test`,
   `npm run test:e2e`.
...
- **Continuity & correctness** preserved: shared example ids, `--role-*` tokens,
  ...
  math outside `src/math`; `docs/ERROR_LOG.md` updated if any visualization bug is
  found.
- **All tests pass** (unit, component, e2e) and `LESSON_CORRECTNESS_CHECKLIST.md`
