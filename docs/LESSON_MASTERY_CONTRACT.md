...
[Approved Insight Contract](./INSIGHT_DISCOVERY_GATE.md) reaches `Gate result:
PASS` and **before** detailed lesson implementation (Gate 5 in the
[workflow](./COURSE_MASTERY_STANDARD.md#9-workflow-integration)).
...
> ([workflow relationship rule](./COURSE_MASTERY_STANDARD.md#9-workflow-integration)).

Read alongside:

- [COURSE_MASTERY_STANDARD.md](./COURSE_MASTERY_STANDARD.md) — profiles, mastery
  dimensions (D1–D14), evidence levels (E1–E7), assessment architecture.
- [LESSON_TEMPLATE.md](./LESSON_TEMPLATE.md) — the *implementation* planning
...
- [ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md) — the pattern library the
  [exercise standard](#3-exercise-and-assessment-standard) below draws on.
- [SEMANTIC_PAGE_GRAMMAR.md](./SEMANTIC_PAGE_GRAMMAR.md) — how the taught content
...
- **Lesson / spine position:** `<e.g. L5 solution-sets>` (link
  [spine](./LINEAR_ALGEBRA_COURSE_SPINE.md) row).
...
demonstrate it at a stated [evidence level](./COURSE_MASTERY_STANDARD.md#5-evidence-levels).
...
- **Mathematical correctness checks:** the invariants/regressions this lesson's
  math must satisfy (link [MATH_CORRECTNESS.md](./MATH_CORRECTNESS.md) /
  [LESSON_CORRECTNESS_CHECKLIST.md](./LESSON_CORRECTNESS_CHECKLIST.md)); note any
...
[ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md), and the course-level layering
(diagnostic, module, mock exams, exam mode) lives in
[COURSE_MASTERY_STANDARD.md §6](./COURSE_MASTERY_STANDARD.md#6-assessment-architecture).
...
Consistent with [ASSESSMENT_PATTERNS.md](./ASSESSMENT_PATTERNS.md#choosing-a-small-assessment-set)
...
module assessment and the [validation pilot](./INSIGHT_VALIDATION_PROTOCOL.md)),
...
- [LESSON_CORRECTNESS_CHECKLIST.md](./LESSON_CORRECTNESS_CHECKLIST.md) is
  completed and any fixed bug is logged in [ERROR_LOG.md](./ERROR_LOG.md).
...
[calibration case](./COURSE_MASTERY_STANDARD.md#8-calibration-cases).
...
failure ([COURSE §8, opposite failures](./COURSE_MASTERY_STANDARD.md#8-calibration-cases)):

- lacking a proof, when the profile is A and a short "why" suffices;
- lacking a real-world application that adds no value;
- not exercising a dimension outside its lesson-level scope ([COURSE §4](./COURSE_MASTERY_STANDARD.md#4-mastery-dimensions));
...
An accepted lesson feeds the [cumulative module assessment](./COURSE_MASTERY_STANDARD.md#9-workflow-integration)
