...
1. **The derivation schema forces a five-part template.** `WorkedStep`
   ...
   contradicts `product/vision.md` §10 ("not a five-line annotation on
...
- **Authoring level (the real pressure):** the five labels are **hard-coded strings**
  ...
  (`docs/archive/lesson-depth-pattern.md:78`), and `product/vision.md` §10 + §17
...
generalized ahead of a second consumer — the exact risk `archive/lesson-depth-pattern.md`
...
3. `src/lessons/eigenvectors.ts:72-174` — re-author the worked-example steps to the
   ...
   rungs — see §4).
4. `docs/product/vision.md` §10 + §17 and
   `docs/archive/lesson-depth-pattern.md:78` — relax the five-element requirement to
...
- **Arrow direction is stabilized** to one hemisphere (`stabilizeDirection`,
  ...
  was a logged bug fix (`quality/known-failure-modes.md` 2026-07-19 eigen scene entry).
...
- **Labels overlap/disappear:** the explorer draws at most two eigenlines +
  ...
  (`quality/known-failure-modes.md` two 2026-07-20 entries). Explorer label collisions near the origin
...
- **Mathematical-correctness risks:** a morph that visually "moves `λv` across the
  ...
  (`engineering/math-correctness.md` visual-interpolation section, `authoring/animation-quality-bar.md`
...
### Is `authoring/animation-quality-bar.md` referenced by implementation or workflow?
...
- `docs/archive/lesson-depth-pattern.md` explicitly frames Lesson 4 as the **reference to
...
- **Generalizing a DSL from one lesson.** `WorkedStep`/`DepthLayer`/`AuthoredCallout`
  ...
  consumer" risk named in `archive/lesson-depth-pattern.md:92`.
...
4. **Decide whether the defective/repeated case should be reachable by exploration**
   ...
   intended behavior.
5. **Wire `authoring/animation-quality-bar.md` into review** (a PR checklist or a lightweight
...
| Trajectory | Reusable infra on one flagship lesson | Medium-High | recent additions all Lesson-4 depth; `archive/lesson-depth-pattern.md` | Reconcile scope guard vs. where work is going? |
