...
`docs/authoring/lesson-design.md`), not a full WCAG audit:
...
- Non-color cues: guided scenes and explorers already pair color roles with
  labels/line-style per `authoring/lesson-design.md`; unchanged in M6.
...
- `e2e/motion-canvas-spike.spec.ts` — `counters()` helper updated: the debug
  ...
  code split** and is documented here rather than in `quality/known-failure-modes.md` (no
