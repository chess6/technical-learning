...
[authoring/lesson-design.md](../authoring/lesson-design.md) specifies *how* a lesson is built (phase
...
authoring/lesson-design.md — it cross-references them. When this document and
authoring/lesson-design.md appear to overlap (e.g. "one conceptual change at a time"), this
document explains the *learning reason*; authoring/lesson-design.md specifies the
*production rule*.
...
- **Watch before Explore (default).** The learner normally sees a guided
  ...
  (product/semantic-page-grammar.md §2) — that is discovery, not an unguided playground.
...
**Notation.** Follow authoring/lesson-design.md and engineering/math-correctness.md exactly (KaTeX,
...
- **Respect the establishing frame as an orienting act.** The paused first frame
  ...
  question. (Mechanics in authoring/lesson-design.md; the *reason* is that orientation
...
Honesty constraint (durable, and echoing engineering/math-correctness.md): **an educational
...
alone, per authoring/lesson-design.md accessibility):
...
- **Gamification.** No points, streaks, or badges; motivation comes from the ideas
  (consistent with authoring/lesson-design.md navigation rules).
...
[quality/lesson-correctness-checklist.md](../quality/lesson-correctness-checklist.md) and the
authoring/lesson-design.md acceptance checklist. This one checks **educational depth**.
